...
export function ManualEntryModal({
  ...
  useEffect(() => {
    ...
    setCategoryId(categories.find((category) => category.name === "Uncategorized")?.id.toString()
        ...
        <form className="space-y-5 p-5 sm:p-6" onSubmit={handleSubmit}>
          {(accounts.length === 0 || categories.length === 0) && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900" role="status">
              Load at least one existing account and category before creating a manual entry.
            </div>
          )}
